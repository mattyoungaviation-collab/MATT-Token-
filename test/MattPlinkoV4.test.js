...
describe("MattPlinkoV4", function () {
  ...
  beforeEach(async function () {
    ...
    plinko = await (await ethers.getContractFactory("MattPlinkoV4")).deploy(
  ...
  it("uses the symmetric V4 payout scale", async function () {
    const expected = [
      500000, 250000, 100174, 50000, 20000, 15000, 8000, 7000, 4848,
      7000, 8000, 15000, 20000, 50000, 100174, 250000, 500000
    ];
  ...
  it("has an exact 98.2% theoretical RTP", async function () {
    const combinations = [
    ...
    expect(weighted).to.equal(643563520n);
    expect(await plinko.RTP_NUMERATOR()).to.equal(weighted);
    expect(await plinko.RTP_DENOMINATOR()).to.equal(655360000n);
    expect(Number(weighted) / (65536 * 10000)).to.equal(0.982);
  });
