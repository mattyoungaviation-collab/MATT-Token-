...
module.exports = async ({ deployments, getNamedAccounts, network }) => {
  ...
  const verifier = process.env.X_REWARD_VERIFIER_ADDRESS || "";
  const owner = process.env.DAILY_REWARDS_OWNER || deployer;

  for (const [name, value] of Object.entries({ mattToken, coinFlip, verifier, owner })) {
    if (!value || !ethers.isAddress(value)) {
      throw new Error(`Set ${name} to a valid 0x address before deploying MattDailyRewardsV2`);
    }
  }
  ...
  console.log(`X proof verifier: ${verifier}`);
  console.log(`Daily rewards owner: ${owner}`);

  await deploy("MattDailyRewardsV2", {
    from: deployer,
    args: [mattToken, coinFlip, verifier, owner],
    log: true,
    waitConfirmations: network.name === "ronin" ? 5 : 1,
  });
};

module.exports.tags = ["MattDailyRewardsV2"];