...
async function main() {
  if (!process.env.PLINKO_V3_ADDRESS) throw new Error("PLINKO_V3_ADDRESS is required.");
  const address = hre.ethers.getAddress(process.env.PLINKO_V3_ADDRESS);
  const contract = await hre.ethers.getContractAt("MattPlinkoV3", address);
