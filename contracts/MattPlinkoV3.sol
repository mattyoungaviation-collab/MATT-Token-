...
interface IRoninVRFCoordinatorForPlinkoV3 {
...
contract MattPlinkoV3 is Ownable, Pausable, ReentrancyGuard {
    ...
    IRoninVRFCoordinatorForPlinkoV3 public immutable vrfCoordinator;
        ...
        Ownable(treasuryAddress)
    {
        ...
        vrfCoordinator = IRoninVRFCoordinatorForPlinkoV3(coordinatorAddress);
        _pause();
    }
    ...
    /// @notice Board RTP is exactly 92.6007080078125%.
    /// 50x, 25x, 10x, 5x, 2x, 1.5x, .8x, .7x, .2x,
    /// .7x, .8x, 1.5x, 2x, 5x, 10x, 25x, 50x.
    function multiplierForSlot(uint8 slot) public pure returns (uint256) {
        ...
        if (slot == 2 || slot == 14) return 100_000;
