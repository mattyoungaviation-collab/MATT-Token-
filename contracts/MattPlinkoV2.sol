...
interface IRoninVRFCoordinatorForPlinkoV2 {
...
contract MattPlinkoV2 is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public constant MULTIPLIER_SCALE = 10_000;
    uint256 public constant MAX_MULTIPLIER = 2_000_000; // 200x
    ...
    IRoninVRFCoordinatorForPlinkoV2 public immutable vrfCoordinator;
        ...
        Ownable(treasuryAddress)
    {
        ...
        vrfCoordinator = IRoninVRFCoordinatorForPlinkoV2(coordinatorAddress);
        _pause();
    }
    ...
    /// @notice Buys 1–100 fixed-price Plinko coins and requests one seed for the batch.
    /// @dev Excess RON sent above the live VRF cost is refunded by the coordinator.
    ...
    /// @dev All outcomes are fixed here before the first browser animation begins.
    ...
    /// @notice Credits the original batch wager if Ronin VRF does not fulfill in time.
    ...
    /// @notice Maps 16 independent VRF-derived bits to a physical 17-slot board.
    ...
    /// @notice Sustainable board:
    /// 200x, 162x, 38x, 9x, 3x, 1.5x, .5x, .25x, .1x,
    /// .25x, .5x, 1.5x, 3x, 9x, 38x, 162x, 200x.
    function multiplierForSlot(uint8 slot) public pure returns (uint256) {
        if (slot >= SLOT_COUNT) revert InvalidSlot();
        if (slot == 0 || slot == 16) return 2_000_000;
        if (slot == 1 || slot == 15) return 1_620_000;
        if (slot == 2 || slot == 14) return 380_000;
        if (slot == 3 || slot == 13) return 90_000;
        if (slot == 4 || slot == 12) return 30_000;
        if (slot == 5 || slot == 11) return 15_000;
        if (slot == 6 || slot == 10) return 5_000;
        if (slot == 7 || slot == 9) return 2_500;
        return 1_000;
    }
