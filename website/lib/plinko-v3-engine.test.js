...
const engine = require("../public/plinko-v3-engine");

assert.equal(engine.ROWS, 16);
assert.equal(engine.SLOT_COUNT, 17);
assert.deepEqual(engine.MULTIPLIERS, [
  50, 25, 10, 5, 2, 1.5, 0.8, 0.7, 0.2,
  0.7, 0.8, 1.5, 2, 5, 10, 25, 50
]);
assert.equal(engine.boardRtp(), 0.926007080078125);
...
assert.equal(engine.payoutForSlot(8), 2_000);
assert.equal(
...
console.log("Plinko V3 visual/result engine tests passed.");
