(() => {
  ...
  const engine = window.MattPlinkoV4Engine;
  if (!engine) throw new Error("MATT Plinko V4 result engine did not load.");
  ...
  const PLINKO_V4_ADDRESS = "0xFF2E30F4EFe8aF8E32365fCba76dCc9136006822";
  ...
  const deployed = ethers.isAddress(PLINKO_V4_ADDRESS) && PLINKO_V4_ADDRESS !== ethers.ZeroAddress;
  ...
  function errorMessage(error) {
    ...
    if (/insufficientbankroll/i.test(message)) return "The V4 bankroll cannot safely cover this batch. Try fewer coins.";
  ...
  async function connectWallet() {
    ...
    if (deployed) state.contract = new ethers.Contract(PLINKO_V4_ADDRESS, PLINKO_ABI, state.signer);
  ...
  async function refreshAccount() {
    ...
    renderClaimable();
    $("#bankroll-balance").textContent = deployed ? `${formatMatt(bankroll)} MATT` : "V4 PENDING";
  ...
  function renderClaimable() {
    if (!deployed) {
      $("#claimable-balance").textContent = "V4 PENDING";
      return;
    }
  ...
  async function initContract() {
    ...
    state.readContract = new ethers.Contract(PLINKO_V4_ADDRESS, PLINKO_ABI, state.readProvider);
    if (await state.readProvider.getCode(PLINKO_V4_ADDRESS) === "0x") {
      throw new Error("No V4 contract exists at the configured address.");
    }
    state.paused = await state.readContract.paused();
    $("#mode-pill").textContent = state.paused ? "V4 PAUSED" : "LIVE ON RONIN";
    $("#contract-link").href = `https://explorer.roninchain.com/address/${PLINKO_V4_ADDRESS}`;
    $("#contract-link").textContent = "VERIFY V4 CONTRACT";
  ...
  function updatePurchaseSummary() {
    ...
    $("#max-batch-payout").textContent = `${(selectedTotal() * 50).toLocaleString()} MATT`;
  ...
  function updateControls() {
    ...
    $("#purchase-button").textContent = !deployed
      ? "V4 CONTRACT DEPLOYMENT PENDING"
      : state.paused
        ? "V4 IS PAUSED"
        : state.purchasing
          ? "LOADING MATT COINS…"
  ...
  async function waitForAllowance(amount) {
    ...
    for (let attempt = 0; attempt < 12; attempt += 1) {
      if (await readToken.allowance(state.account, PLINKO_V4_ADDRESS) >= amount) return;
      await delay(1_000);
    }
  ...
  async function purchaseBatch() {
    ...
    try {
      ...
      const allowance = await state.token.allowance(state.account, PLINKO_V4_ADDRESS);
      if (allowance < amount) {
        setStatus(`Approve exactly ${selectedTotal().toLocaleString()} MATT for this batch.`);
        const approval = await state.token.approve(PLINKO_V4_ADDRESS, amount);
    ...
    for (let attempt = 0; attempt < RESULT_POLL_ATTEMPTS; attempt += 1) {
      ...
      if (Number(batch.status) === 2) {
        ...
        if (slots.some(slot => slot < 0 || slot >= engine.SLOT_COUNT)) {
          throw new Error("The contract returned an invalid V4 slot.");
        }
  ...
  function storageKey() {
    return state.account ? `mattPlinkoV4:${state.account.toLowerCase()}` : null;
  }
  ...
  async function resumeSavedBatch() {
    ...
    setStatus("Restoring your saved V4 batch…");
  ...
  async function withdraw() {
    ...
    updateControls();
    try {
      setStatus("Confirm the V4 winnings withdrawal in Ronin Wallet.");
      const transaction = await state.contract.withdraw();
      await transaction.wait(1);
      setStatus("Your V4 Plinko winnings were sent to your wallet.", "good");
      await refreshAccount();
    } finally {
      state.purchasing = false;
      updateControls();
