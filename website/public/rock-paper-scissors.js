...
if (String(chainId).toLowerCase() === CHAIN_HEX) return provider;
try {
...
const injectedRonin = [window.ronin?.provider, window.ronin].find(candidate => candidate && typeof candidate.request === "function");
injectedRonin?.on?.("accountsChanged", () => location.reload());
injectedRonin?.on?.("chainChanged", () => location.reload());
